...
/**
 * A valid-but-absent image (e.g. a `golden` requested for a record-mode key, where getImageChunk
 * returns null immediately) must produce a descriptive 404, not an empty 200 with a broken image.
 */
fun imageEndpointAbsentImageReturns404Test() {
    val api: ScreenshotTestApi = object : ScreenshotTestApi {
        ...
        // The key/kind is recognized but has no bytes -> null at the first slice.
    ...
    try {
        ...
        val conn = URL("http://localhost:$port/image?id=sess-rec&key=summary&kind=golden").openConnection() as HttpURLConnection
        assertEquals(404, conn.responseCode, "Expected HTTP 404 for an absent image; got ${conn.responseCode}")
        val body = (conn.errorStream ?: conn.inputStream).bufferedReader().readText()
        assertEquals(
            "No golden image found for session \"sess-rec\", key \"summary\".",
            body,
            "Expected the full descriptive absent-image 404 body; got:\n$body"
        )
    } finally {
        server.stop()
    }
