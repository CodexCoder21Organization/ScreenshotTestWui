 ...
 * page named by the returnTo field with an enumerated notice; that page composes the confirmation
 * banner itself. A blank reason falls back to the documented default.
 */
fun cancelSessionRecordsReasonAndRedirectsWithNoticeTest() {
    val sessionsJson = """
        [
    ...
    try {
        ...
        // From the workers page, with an operator-supplied reason.
        val fromWorkers = post(port, "/session/cancel", "id=sess-q2&reason=Superseded+by+a+newer+run&returnTo=workers")
        assertEquals(303, fromWorkers.responseCode, "Expected 303 See Other after a successful cancel; body:\n${bodyOf(fromWorkers)}")
        assertEquals("/workers?notice=cancelled&noticeId=sess-q2", fromWorkers.getHeaderField("Location"))
        assertEquals(listOf("sess-q2" to "Superseded by a newer run"), cancels.toList(), "Expected exactly one cancelSession call with the submitted reason.")
        ...
        val fromList = post(port, "/session/cancel", "id=sess-r1&reason=++&returnTo=list")
        assertEquals(303, fromList.responseCode)
        assertEquals("/?notice=cancelled&noticeId=sess-r1", fromList.getHeaderField("Location"))
        assertEquals("sess-r1" to "Cancelled from the management UI", cancels.last(), "Expected the default reason for a blank one.")

        // From a session's own page: returns to that page, with the id URL-encoded.
        val fromSession = post(port, "/session/cancel", "id=sess+x%2F1&returnTo=session")
        assertEquals(303, fromSession.responseCode)
        assertEquals("/session?id=sess%20x%2F1&notice=cancelled&noticeId=sess%20x%2F1", fromSession.getHeaderField("Location"))
        assertEquals("sess x/1" to "Cancelled from the management UI", cancels.last(), "Expected a missing reason to use the default.")

        // An unrecognised returnTo falls back to the sessions list (never an arbitrary URL).
