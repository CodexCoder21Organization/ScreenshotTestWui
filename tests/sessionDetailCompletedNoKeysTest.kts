...
/**
 * A COMPLETED session whose results contain zero image keys must render a clear "no keys" placeholder
 * and no results table (rather than an empty table or a crash).
 */
fun sessionDetailCompletedNoKeysTest() {
    val statusJson = """{"sessionId":"sess-empty","state":"COMPLETED","error":null,"rendererVersion":"chromium-1228"}"""
    val resultsJson = """{"rendererVersion":"chromium-1228","mode":"compare","results":[]}"""
    ...
    try {
        ...
        val conn = URL("http://localhost:$port/session?id=sess-empty").openConnection() as HttpURLConnection
        assertEquals(200, conn.responseCode, "Expected HTTP 200 for a COMPLETED zero-key session; got ${conn.responseCode}")
        val html = conn.inputStream.bufferedReader().readText()
        assertTrue(html.contains(">COMPLETED<"), "Expected a COMPLETED state badge.")
        assertTrue(
            html.contains("This session produced no image keys."),
            "Expected the zero-keys placeholder; page was:\n$html"
        )
        assertFalse(html.contains("id=\"results-table\""), "A zero-key session must not render a results table.")
    } finally {
        server.stop()
    }
}
