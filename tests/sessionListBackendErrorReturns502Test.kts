...
/**
 * When the backend listSessions call fails, the landing page must answer a non-200 (502 Bad Gateway)
 * so health checks and monitors see the failure — not a misleading 200 with an error body — and the
 * page must carry the descriptive failure reason.
 */
fun sessionListBackendErrorReturns502Test() {
    val api: ScreenshotTestApi = object : ScreenshotTestApi {
        ...
        override fun listSessions(): String = throw RuntimeException("url://screenshottest/ is unreachable")
    ...
    try {
        ...
        assertEquals(502, conn.responseCode, "Expected HTTP 502 when the backend is unreachable; got ${conn.responseCode}")
        val body = (conn.errorStream ?: conn.inputStream).bufferedReader().readText()
        assertTrue(
            body.contains("Failed to load sessions: url://screenshottest/ is unreachable"),
            "Expected the descriptive backend-failure message on the page; body was:\n$body"
        )
    } finally {
        server.stop()
    }
