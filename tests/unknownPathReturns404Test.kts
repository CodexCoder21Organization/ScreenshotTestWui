...
/**
 * The sessions list is bound to the context-root mapping ("") rather than the catch-all default ("/"),
 * so an unmapped path like /favicon.ico gets a plain 404 instead of triggering a listSessions RPC on
 * every browser request. This guards that mapping choice.
 */
fun unknownPathReturns404Test() {
    val api: ScreenshotTestApi = object : ScreenshotTestApi {
        ...
        // If /favicon.ico were routed to the list servlet this would blow up the request.
        override fun listSessions(): String = throw IllegalStateException("listSessions must not be called for an unmapped path")
    ...
    try {
        ...
        val conn = URL("http://localhost:$port/favicon.ico").openConnection() as HttpURLConnection
        assertEquals(404, conn.responseCode, "Expected HTTP 404 for an unmapped path (/favicon.ico); got ${conn.responseCode}")
    } finally {
        server.stop()
    }
}
