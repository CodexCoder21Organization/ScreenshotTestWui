...
/**
 * Cancelling a session that is no longer RUNNING: the service's IllegalStateException is surfaced as
 * 409 Conflict on the page the action came from, in an error banner carrying the service's full
 * message; an unknown id (IllegalArgumentException) is 404; any other backend failure is 502.
 */
fun cancelTerminalSessionShowsBackendConflictTest() {
    val sessionsJson = """
        [
